//src//pages/api/participant/joinlink.ts
//store the participant data only once even they clcik multiple times

import { NextApiRequest, NextApiResponse } from 'next';
import { PrismaClient, JoinedVia } from '@prisma/client';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Only authenticated user can perform actions
        if (req.method === 'POST') {
            return await joinActivityViaLink(req, res, user.id); // Pass authenticated user ID
        } else 
        // if (req.method === 'GET') {
...
export async function joinActivityViaLink(req: NextApiRequest, res: NextApiResponse, userId: number) {
    if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

    const { inviteCode } = req.body;
    console.log("Received request to join with inviteCode:", inviteCode, "by userId:", userId);

    if (!inviteCode) return res.status(400).json({ error: 'Invite code is required' });

    try {
        // Validate if the invite link exists
        ...
        // Prepare the receivedBy JSON data
        ...
        // Merge the new participant with existing ones
        const existingParticipants = Array.isArray(invite.receivedBy) ? invite.receivedBy : [];
        const updatedParticipants = [...existingParticipants, newParticipant];

        await prisma.inviteLink.update({
            where: { id: invite.id },
            data: { receivedBy: updatedParticipants },
        });
        console.log("Updated receivedBy JSON with new participant data");
...
//         }
//         if (req.method === 'POST') {
//             return await joinActivityViaLink(req, res);
